...
class logger {
	int port;
	int rotation;
	int kill1, kill2, kill3, kill4, kill5;
	string tag;
	string filter;
	string nofilter;
...
public:
	...
	static void listener_function_parent (logger &log) {
		...
		log.listenhandle = socket(AF_INET, SOCK_STREAM, 0);
		// creating a socket for sever
		if (log.listenhandle < 0) {
		...
		serv_addr.sin_port = htons(log.port);
		/* Now bind the host address using bind() call.*/
		if (bind(log.listenhandle, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0) {
		...
		setsockopt(log.listenhandle, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv,sizeof(struct timeval));
		listen(log.listenhandle,MAX_LISTEN);
		...
		while (1) {
			...
			int newsockfd = accept_client(log.listenhandle, 30);
...
//			allth.emplace_back(&listener_function, std::ref(log), newsockfd);
	...
	static void listener_function (logger &log, int sock) {
		...
		while (1) {
			...
			log.incoming_messages.push_back(string(buffer));
			log.mu_for_message.unlock();
		}
		close(sock);
		log.kill2 =1;
